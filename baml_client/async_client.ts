...
import type {MediaType, MessageStatus, Resume, WhatsAppMessage, WhatsAppScreenshot} from "./types"
...
export type RecursivePartialNull<T> = MovedRecursivePartialNull<T>
    ...
    env?: Record<string, string | undefined>
      tags?: Record<string, string>
        ...
        async ExtractWhatsAppMessages(
        screenshot: Image,screenshot_filename: string,
        __baml_options__?: BamlCallOptions<never>
        ): Promise<types.WhatsAppScreenshot> {
          try {
          const options = { ...this.bamlOptions, ...(__baml_options__ || {}) }
          const signal = options.signal;
          ...
          // Check if onTick is provided - route through streaming if so
          if (options.onTick) {
          const stream = this.stream.ExtractWhatsAppMessages(
          ...
          const env: Record<string, string> = Object.fromEntries(
            ...
            {
            "screenshot": screenshot,"screenshot_filename": screenshot_filename
            },
            this.ctxManager.cloneContext(),
            options.tb?.__tb(),
            options.clientRegistry,
            ...
            return raw.parsed(false) as types.WhatsAppScreenshot
            } catch (error) {
            throw toBamlError(error);
            }
            }
            ...
            screenshot: Image,screenshot_filename: string,
            __baml_options__?: BamlCallOptions<never>
            ): BamlStream<partial_types.WhatsAppScreenshot, types.WhatsAppScreenshot>
              {
              try {
              const options = { ...this.bamlOptions, ...(__baml_options__ || {}) }
              const signal = options.signal;
              ...
              console.error("Error in onTick callback for ExtractWhatsAppMessages", error);
              }
              }
              };
              }
              ...
              const env: Record<string, string> = Object.fromEntries(
                ...
                return new BamlStream<partial_types.WhatsAppScreenshot, types.WhatsAppScreenshot>(
                  raw,
                  (a): partial_types.WhatsAppScreenshot => a,
                  (a): types.WhatsAppScreenshot => a,
                  this.ctxManager.cloneContext(),
                  options.signal,
                  )
                  } catch (error) {
